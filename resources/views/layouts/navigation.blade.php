<nav class="border-b border-brand-200 bg-white/90 backdrop-blur">
    <div class="mx-auto flex h-16 max-w-6xl items-center justify-between px-4 sm:px-6 lg:px-8">
        <div class="flex items-center gap-8">
            ...
            @auth
                <div class="hidden items-center gap-6 text-sm font-medium text-brand-500 md:flex">
                    ...
                    <a href="{{ route('profile.edit') }}" class="hover:text-brand-900 {{ request()->routeIs('profile.*') ? 'text-brand-900' : '' }}">Profil</a>
                </div>
            @endauth
        </div>
        ...
        <div class="flex items-center gap-3 text-sm">
            @auth
                <span class="hidden text-brand-500 md:inline">{{ Auth::user()->name }}</span>
